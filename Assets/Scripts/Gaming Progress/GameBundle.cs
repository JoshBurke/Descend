...
public class GameFolder
{
    ...
    public GameDataBase DeserializeDataBase(string fileName, Type dataBaseType)
    {
        GameDataBase tempDataBase;
        if (this.DoesXmlFileExist(fileName)) {
            fileName += ".xml";
            string filePathOnDisk = Path.Combine(this.Location, fileName);
            XmlSerializer xs = new XmlSerializer(dataBaseType);
        ...
        {
            tempDataBase = (GameDataBase)Activator.CreateInstance(dataBaseType);
        }
